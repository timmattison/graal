    ...
    protected int onLoadCallback(JNIJavaVM vm, JvmtiEnv jvmti, JvmtiEventCallbacks callbacks, String options) {
        ...
        if (configOutputDir != null) {
            ...
            try {
                ...
                configOutputLockFilePath = configOutputDirPath.resolve(ConfigurationFile.AGENT_LOCK_FILE_NAME);
                try {
                    ...
                    return error(2, "Output directory '" + configOutputDirPath + "' is locked by process " + process + ", " +
                                    ...
                                    "Unless file '" + ConfigurationFile.AGENT_LOCK_FILE_NAME + "' is a leftover from an earlier process that terminated abruptly, it is unsafe to delete it. " +
                                    "For running multiple processes with agents at the same time to create a single configuration, read Agent.md on how to use the native-image-configure tool.");
                }
